...
// 2. Class<*> instances are excluded from instance dumps.
// These assumptions are correct when reading dumps generated by HeapDumper.
class HeapRestorer : public StackObj {
 public:
  ...
  void restore_heap(TRAPS) {
    // For now we rely on CDS to pre-create new built-in class loaders for us
    ...
    // TODO restore objects from roots

    if (HAS_PENDING_EXCEPTION) {
      return;
    }

    postcond(SystemDictionary::java_platform_loader() != nullptr);
    postcond(SystemDictionary::java_system_loader() != nullptr);
  }
 ...
 private:
  ...
  Symbol *get_dumped_class_name(HeapDumpFormat::id_t class_id, TRAPS) {
  ...
  HeapDumpFormat::id_t _platform_class_loader_id;
  HeapDumpFormat::id_t _builtin_app_class_loader_id;
  HeapDumpFormat::id_t _system_class_loader_id;

#define CHECK_ABSENT_AND_SET(id_ptr, value, klass, field_name)                                \
  if (*id_ptr != 0) {                                                                         \
    ResourceMark rm;                                                                          \
    char *class_name = klass->name()->as_C_string();                                          \
    THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(),                               \
               err_msg("Static field %s.%s is dumped twice", class_name, field_name), false); \
  }                                                                                           \
  *id_ptr = value.as_object_id

  // Finds platform and system class loaders' IDs in the dump.
  void find_base_class_loader_ids(TRAPS) {
    // This relies on the ClassLoader.get*ClassLoader() implementation details:
    // - The system class loader is in java.lang.ClassLoader's scl static field
    // - The built-in platform/app class loader is in
    // jdk.internal.loader.ClassLoaders' PLATFORM_LOADER/APP_LOADER static field
    ...
    _heap_dump.load_class_records.iterate([&](HeapDumpFormat::id_t _, const HeapDumpFormat::LoadClassRecord &lcr) -> bool {
      ...
      if (name == vmClasses::ClassLoader_klass()->name()) {
        HeapDumpFormat::ClassDumpRecord *dump = get_class_dump(lcr.class_id, CHECK_false);
        if (dump->class_loader_id != 0) {
          ResourceMark rm;
          THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(),
                     err_msg("Class %s can only be loaded by the bootstrap class loader",
                             vmClasses::ClassLoader_klass()->name()->as_C_string()), false);
        }
        ...
        for (size_t i = 0; i < dump->static_fields.size(); i++) {
          ...
          Symbol *field_name = get_dumped_symbol(field_dump.info.name_id, CHECK_false);
          if (field_name->equals(SCL_FIELD_NAME)) {
            CHECK_ABSENT_AND_SET(&_system_class_loader_id, field_dump.value, vmClasses::ClassLoader_klass(), SCL_FIELD_NAME);
            if (_system_class_loader_id == 0) {
              THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(), "Dumped system class loader is null", false);
            }
          }
        }
      } else if (name == vmClasses::jdk_internal_loader_ClassLoaders_klass()->name()) {
        const HeapDumpFormat::ClassDumpRecord *dump = get_class_dump(lcr.class_id, CHECK_false);
        if (dump->class_loader_id != 0) {
          return true;  // Continue: wrong class (the real one is boot-loaded)
        }
        ...
        for (size_t i = 0; i < dump->static_fields.size(); i++) {
          ...
          if (field_name->equals(PLATFORM_LOADER_FIELD_NAME)) {
            CHECK_ABSENT_AND_SET(&_platform_class_loader_id, field_dump.value,
                                 vmClasses::jdk_internal_loader_ClassLoaders_klass(), PLATFORM_LOADER_FIELD_NAME);
            if (_platform_class_loader_id == 0) {
              THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(), "Dumped platform class loader is null", false);
            }
          } else if (field_name->equals(APP_LOADER_FIELD_NAME)) {
            CHECK_ABSENT_AND_SET(&_builtin_app_class_loader_id, field_dump.value,
                                 vmClasses::jdk_internal_loader_ClassLoaders_klass(), APP_LOADER_FIELD_NAME);
            if (_builtin_app_class_loader_id == 0) {
              THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(), "Dumped built-in system class loader is null", false);
            }
          }
        }
      }

      return true;  // Continue: wrong class
    });
    ...
    if (_builtin_app_class_loader_id == 0 ||
      ...
      THROW_MSG(vmSymbols::java_lang_IllegalArgumentException(), "Cannot find dumped built-in system class loader");
    }
    if (_system_class_loader_id == 0 ||
    ...
    if (_platform_class_loader_id == _builtin_app_class_loader_id ||
      ...
      THROW_MSG(vmSymbols::java_lang_IllegalArgumentException(),
                "Platform and system class loaders cannot be the same dumped instance");
    }
  }

#undef CHECK_ABSENT_AND_SET
  ...
  Klass *load_class(const HeapDumpFormat::ClassDumpRecord &dump, TRAPS) {
    ...
    // TODO instead, we must load all classes referenced in the constant pool,
    // because the standard resolution mechanism will be called for them
    // otherwise which may call the user code
    if (dump.super_id != 0) {
  ...
  instanceHandle get_prepared_class_loader(HeapDumpFormat::id_t id, TRAPS) {
    ...
    if (id == 0) {  // Bootstrap class loader
  ...
  static void t(Symbol *runtime_name, Symbol *dumped_name, Symbol *signature, u1 dumped_type) {
  ...
  Klass *restore_class(const HeapDumpFormat::ClassDumpRecord &dump, TRAPS) {
    ...
    if (dump.super_id != 0 && klass->java_super() != nullptr) {
      HeapDumpFormat::ClassDumpRecord *super_dump = get_class_dump(dump.super_id, CHECK_NULL);
    ...
    } else if (dump.super_id == 0 && klass->java_super() != nullptr) {
      ResourceMark rm;
      THROW_MSG_NULL(vmSymbols::java_lang_IllegalArgumentException(),
  ...
  void set_field(oop obj, const FieldStream &fs, const HeapDumpFormat::BasicValue &val, TRAPS) {
    switch (fs.signature()->char_at(0)) {
      ...
      case JVM_SIGNATURE_ARRAY: {
        ...
        if (restored.not_null() && !restored->klass()->is_subtype_of(get_field_klass(fs))) {
          ResourceMark rm;
          THROW_MSG(vmSymbols::java_lang_IllegalArgumentException(),
                    err_msg("Field referencing a %s is dumped as an incompatible %s instance",
                            get_field_klass(fs)->external_name(), restored->klass()->external_name()));
        }
  ...
  static Klass *get_field_klass(const FieldStream &fs) {
    JavaThread *current = JavaThread::current();
    InstanceKlass *field_holder = fs.field_descriptor().field_holder();
    Handle loader = Handle(current, field_holder->class_loader());
    Handle prot_dom = Handle(current, field_holder->protection_domain());

    // find_instance_or_array_klass() does not strip the envelope, so do it here
    ...
    Klass *field_klass = SystemDictionary::find_instance_or_array_klass(current, class_name, loader, prot_dom);
#ifdef ASSERT
    ResourceMark rm;
    assert(field_klass != nullptr,
           "Field classes should have been resolved during the holder's definition (holder %s, field %s %s)",
           field_holder->external_name(), fs.signature()->as_C_string(), fs.name()->as_C_string());
#endif
  ...
  Handle restore_object(HeapDumpFormat::id_t id, TRAPS) {
    if (id == 0) {  // We treat 0 object ID as null, just as HeapDumper does
      return Handle();
    }
  ...
  instanceHandle restore_instance(const HeapDumpFormat::InstanceDumpRecord &dump, TRAPS) {
    ...
    if (klass->is_class_loader_instance_klass()) {
      // TODO restore all but the prepared fields, treat classes field specially
      Unimplemented();
    } else {
  ...
  instanceHandle get_primitive_class_mirror(const HeapDumpFormat::ClassDumpRecord &class_dump,
    ...
    // We rely on java.lang.Class name field to reveal the primitive type
#ifdef ASSERT
    {
      fieldDescriptor fd;
  ...
  objArrayHandle restore_obj_array(const HeapDumpFormat::ObjArrayDumpRecord &dump, TRAPS) {
    ...
    ObjArrayKlass *klass;
    {
      ...
      if (!k->is_objArray_klass()) {
        ResourceMark rm;
        THROW_MSG_(vmSymbols::java_lang_IllegalArgumentException(),
                   err_msg("Object array dump " INT64_FORMAT " has illegal class %s", dump.id, klass->external_name()),
                   {});
      }
...
bool crac::restore_portable(TRAPS) {
  ...
  heap_restorer.restore_heap(CHECK_false);
